...
impl UploadEntry {
    ...
    pub fn command(&self, remote: &str) -> String {
        match self {
            Self::Directory(path) => format!("mkdir -p {}", sh_quote(&format!("{remote}/{path}"))),
            Self::File(file) => upload_command(file, remote),
        }
    }
}
    ...
    fn visit(
        ...
        for entry in entries {
            ...
            if relative.contains(['\r', '\n']) || relative.len() > 512 {
                return Err(RunnerError::InvalidInput(
                    "serial filename is too long or contains a newline".into(),
                ));
            }
            if files.len() >= 1024 {
                return Err(RunnerError::InvalidInput(
...
pub fn upload_command(file: &TextFile, remote: &str) -> String {
    let path = format!("{}/{}", remote.trim_end_matches('/'), file.relative);
    ...
    let body = encoded
        ...
        .join("\n");
    format!(
        "mkdir -p {} && base64 -d > {} <<'RR_UPLOAD_EOF'\n{body}\nRR_UPLOAD_EOF\n",
        sh_quote(parent),
        sh_quote(&path)
    )
}
...
mod tests {
    ...
    fn upload_payload_is_literal_and_preserves_no_final_newline() {
        ...
        let command = upload_command(&file, "/tmp/work space");
        let encoded = command
            .split("<<'RR_UPLOAD_EOF'\n")
            .nth(1)
            .unwrap()
